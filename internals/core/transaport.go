...
import (
	...
	"github.com/nile-squad/nylonpay-go/internals/crypto"
...
func (t *Transport) Send(ctx context.Context, req models.TransportRequest, out any) error {
	payloadMap, err := structToMap(req.Payload)
	if err != nil {
		return &models.SDKError{Category: "internal", Message: "Failed to parse payload"}
	}
	...
	if err != nil {
		return &models.SDKError{Category: "internal", Message: "Failed to encode envelope"}
	}
	...
	if err != nil {
		return &models.SDKError{Category: "internal", Message: "Failed to sign request"}
	}
	...
	for attempt := 0; attempt <= maxAttempts; attempt++ {
		...
		if err != nil {
			cancel()
			return &models.SDKError{Category: "internal", Message: "Failed to create request"}
		}
		...
		if err != nil {
			...
			return &models.SDKError{Category: category, Message: msg, Retryable: true}
		}
		...
		if err != nil {
			return &models.SDKError{Category: "internal", Message: "Failed to read response body"}
		}

		if resp.StatusCode < 200 || resp.StatusCode >= 300 {
			retryable := models.RetryableStatusCodes[resp.StatusCode]
		...
		var envResp models.BackendResponse
		if err := json.Unmarshal(respBody, &envResp); err != nil {
			return &models.SDKError{Category: "internal", Message: "Response missing status field", Retryable: false}
		}
		...
		if err != nil {
			return &models.SDKError{Category: "internal", Message: "Failed to process response data", Retryable: false}
		}

		if sig == "" {
			return &models.SDKError{Category: "internal", Message: "Response signature missing", Retryable: false}
		}

		isValid, err := crypto.VerifyResponseSignature(strippedMap, sig, t.config.APISecret)
		if err != nil || !isValid {
			return &models.SDKError{Category: "internal", Message: "Response signature verification failed", Retryable: false}
		}

		strippedBytes, _ := json.Marshal(strippedMap)
		if err := json.Unmarshal(strippedBytes, out); err != nil {
			return &models.SDKError{Category: "internal", Message: "Failed to map response to struct", Retryable: false}
		}

		return nil
	}
	return &models.SDKError{Category: "timeout", Message: "Max retries exceeded"}
